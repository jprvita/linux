...
static void bt_device_init(struct bt_dev_data *pdata)
{
    ...
    if(pdata->gpio_wake > 0 ) {
        gpio_request(pdata->gpio_wake, BT_RFKILL);
        gpio_direction_output(pdata->gpio_wake, 1);
    }
...
/* FIXME: maybe define the names on the pin declarations use them to get the numbers? */
static int _gpio_amlogic_name_to_num(const char *name)
{
	if(!strcmp(name,"GPIOX_11"))
		return 11;
	if(!strcmp(name,"GPIOX_20"))
		return 20;
	printk(KERN_WARNING "Unknown GPIO number in bt-dev\n");
	return -1;
}
...
static int bt_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		if(ret){
		    ...
		    pdata->gpio_reset = _gpio_amlogic_name_to_num(str);
		}
		...
		if(ret){
		    ...
		    pdata->gpio_en = _gpio_amlogic_name_to_num(str);
		}
		...
		if(ret){
		    ...
		    pdata->gpio_wake = _gpio_amlogic_name_to_num(str);
		}
	}
#else
